...
class egg_qt4(BaseEggBuilder):
    ...
    def script_substitutions(self):
        ...
        qscis    = list(recursive_glob_as_dict(pysci_.qsci_dir, Pattern.sciapi,
                        strip_keys=True, prefix_key="qsci").items())
        extra_pyqt4_mods = list(recursive_glob_as_dict(pj(pyqt4_.install_site_dir,"PyQt4"),
                                Pattern.pyall, strip_keys=True, prefix_key="PyQt4").items())

        sip_mods = list(recursive_glob_as_dict(sip_.install_site_dir, Pattern.pyall,
                        strip_keys=True, levels=1).items())
...
class egg_qt4_dev(BaseEggBuilder):
    ...
    def script_substitutions(self):
        ...
        libs = list(recursive_glob_as_dict(qt4_.install_lib_dir,
                    Pattern.qtstalib, strip_keys=True, prefix_key="lib").items())
        # sip files are recursive subdirectories and the union of pyqt4 and...
        sips = list(recursive_glob_as_dict(pyqt4_.install_sip_dir,
                    Pattern.sipfiles, strip_keys=True, prefix_key="sip").items())
        # sources are recursive subdirectories and the union of qt4 and that all (CPP have been removed)...
        srcs = list(recursive_glob_as_dict(qt4_.install_src_dir,
                    Pattern.qtsrc, strip_keys=True, prefix_key="src").items())
        # tra files are recursive subdirectories in qt4
        tra = list(recursive_glob_as_dict(qt4_.install_tra_dir,
                    Pattern.qttransl, strip_keys=True, prefix_key="translations").items())
        # mks files are recursive subdirectories in qt4
        mks = list(recursive_glob_as_dict(qt4_.install_mks_dir,
                    Pattern.qtmkspec, strip_keys=True, prefix_key="mkspecs").items())
        # plugins files are recursive subdirectories in qt4
        plu = list(recursive_glob_as_dict(qt4_.install_plu_lib_dir,
                    Pattern.qtstalib, strip_keys=True, prefix_key="plugins").items())
...
class egg_boost(BaseEggBuilder):
    ...
    def script_substitutions(self):
        ...
        incs = list(recursive_glob_as_dict( boost_.install_inc_dir,
                    Pattern.qtinc, strip_keys=True, prefix_key="include", dirs=True).items())
        inc_dirs = merge_list_dict( incs )
